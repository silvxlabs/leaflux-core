...
def attenuate_all(env: Environment, sol: SolarPosition, extn: float = 0.5, origin: tuple[float, float] = (0.0, 0.0)) -> Irradiance:
    ...
    origin: tuple[float, float]
        ...
        which reproduces the un-anchored result exactly.
    ...
    # the original, un-offset coordinates.
    origin_offset = np.array([origin[0], origin[1], 0.0], dtype=np.float32)

    leaf_area_stack_rot = (r @ (env.leaf_area.leaf_area[:, :3] + origin_offset).T).T # Get rotated x, y, z from LAD grid
    ...
    if env.terrain != None:
        ...
        terrain_area_rot_stack = (r @ (env.terrain.terrain[:, :3] + origin_offset).T).T
    ...
    if env.sensors is not None:
        # Rotate
        sensor_rot = (r @ (env.sensors[:, :3] + origin_offset).T).T
